 // ==UserScript==
    // @name         Quasar Interface & Shield Module
    // @namespace    http://tampermonkey.net/
    // @version      4.6
    // @description  KatipOnline Mobil & Masaüstü Uyumlu Otomasyon Altyapısı
    // @author       Quasar
    // @match        https://katiponline.com/*
    // @match        https://*.katiponline.com/*
    // @icon         https://www.google.com/s2/favicons?sz=64&domain=katiponline.com
    // @grant        none
    ...
    (function() {
        ...
        function applyQuasarShield() {
            ...
            const fakeUserAgents = [
                ...
                // --- Mobil Cihaz Çeşitliliği (Android) ---
        ...
        window.QuasarTypeBot = async function(options = {}) {
            const logStyle = "color:#ff0000; font-weight:bold; font-size:12px; background:#000; padding:2px 5px; border-radius:3px;";
            const l = quasarLanguages[window.quasarLang] || quasarLanguages.TR;
            ...
            if (!kaynakTextarea || !hedefTextarea) {
                console.log(`%c${l.textarea_err}`, logStyle);
                quasarLog(l.textarea_err);
                window.isQuasarBotRunning = false;
                botButonunuSıfırla();
                return false;
            }
            let metin = kaynakTextarea.value || kaynakTextarea.textContent || kaynakTextarea.innerText || "";
            metin = metin.trim().replace(/\s+/g, ' ');
            if (!metin) {
                console.log(`%c${l.empty_err}`, logStyle);
                quasarLog(l.empty_err);
                window.isQuasarBotRunning = false;
                botButonunuSıfırla();
                return false;
            }
            ...
            quasarLog(`${l.ready}: ${metin.split(" ").length} WORDS`);
            let nextErrorAt = randomBetween(settings.minErrorWord, settings.maxErrorWord);
            let currentWord = 0;
            ...
            function getDynamicDelay() {
                const currentWPM = window.quasarBotWPM || 100;
                const totalKeystrokesPerMinute = currentWPM * 6;
                const theoreticalDelay = 60000 / totalKeystrokesPerMinute;
                let browserFrictionCompensation = 1.48;
                if (currentWPM > 150) browserFrictionCompensation = 1.55;
                if (currentWPM > 250) browserFrictionCompensation = 1.65;
                if (currentWPM > 400) browserFrictionCompensation = 1.85;
                let baseDelay = theoreticalDelay / browserFrictionCompensation;
                if (window.quasarHumanMode) {
                    const r = Math.random();
                    if (r < 0.15) baseDelay = baseDelay * 0.65;
                    else if (r > 0.85) baseDelay = baseDelay * 1.45;
                }
                const variationPercent = window.quasarHumanMode ? 0.35 : (currentWPM > 300 ? 0.03 : 0.10);
                const variation = baseDelay * variationPercent;

                const finalDelay = randomBetween(baseDelay - variation, baseDelay + variation);
                return Math.max(0, finalDelay);
            }

            async function pressKey(harf) {
                const isSpace = harf === " ";
                const isUpperCase = !isSpace && harf === harf.toUpperCase() && harf !== harf.toLowerCase();
                const keyCode = isSpace ? 32 : harf.charCodeAt(0);
                const code = isSpace ? "Space" : "Key" + harf.toUpperCase();

                if (isUpperCase) {
                    hedefTextarea.dispatchEvent(new KeyboardEvent("keydown", { key: "Shift", code: "ShiftLeft", keyCode: 16, which: 16, bubbles: true }));
                }

                hedefTextarea.dispatchEvent(new KeyboardEvent("keydown", { key: harf, code, keyCode, which: keyCode, bubbles: true, cancelable: true }));
                hedefTextarea.dispatchEvent(new InputEvent("beforeinput", { bubbles: true, cancelable: true, inputType: "insertText", data: harf }));
                document.execCommand("insertText", false, harf);
                hedefTextarea.dispatchEvent(new InputEvent("input", { bubbles: true, cancelable: true, inputType: "insertText", data: harf }));
                hedefTextarea.dispatchEvent(new KeyboardEvent("keyup", { key: harf, code, keyCode, which: keyCode, bubbles: true, cancelable: true }));
            ...
            async function pressBackspace() {
                hedefTextarea.dispatchEvent(new KeyboardEvent("keydown", { key: "Backspace", code: "Backspace", keyCode: 8, which: 8, bubbles: true, cancelable: true }));
                document.execCommand("delete", false, null);
                hedefTextarea.dispatchEvent(new InputEvent("input", { bubbles: true, cancelable: true, inputType: "deleteContentBackward" }));
                hedefTextarea.dispatchEvent(new KeyboardEvent("keyup", { key: "Backspace", code: "Backspace", keyCode: 8, which: 8, bubbles: true, cancelable: true }));
            }
            ...
            for (let i = 0; i < kelimeler.length; i++) {
                // --- EKLENEN KONTROL ---
                const isStillVisible = hedefTextarea && document.body.contains(hedefTextarea) && hedefTextarea.offsetParent !== null;
                if (!window.isQuasarBotRunning || !isStillVisible) {
                    window.isQuasarBotRunning = false;
                    botButonunuSıfırla();
                    return false;
                }
                ...
                quasarLog(`${l.inject}: ${currentWord}/${kelimeler.length}`);

                const shouldMakeHumanError = window.quasarHumanMode && (currentWord >= nextErrorAt);
                let errorPosition = -1;
                let fakeLetter = "";

                if (shouldMakeHumanError) {
                    nextErrorAt += randomBetween(settings.minErrorWord, settings.maxErrorWord);
                    errorPosition = Math.floor(randomBetween(1, Math.max(2, kelime.length - 1)));
                    fakeLetter = randomLetter();
                    quasarLog(`${l.typo} @ WD ${currentWord}`);
                }

                for (let charIndex = 0; charIndex < kelime.length; charIndex++) {
                    if (!window.isQuasarBotRunning) return false;

                    typedChars++;
                    if (typedChars % settings.refocusEvery === 0) {
                        hedefTextarea.focus();
                    }

                    if (shouldMakeHumanError && charIndex === errorPosition) {
                        await pressKey(fakeLetter);
                        await sleep(getDynamicDelay() * randomBetween(1.1, 1.5));
                        await pressBackspace();
                        await sleep(getDynamicDelay() * 1.3, 1.8);
                    }

                    const harf = kelime[charIndex];
                    await pressKey(harf);

                    const delay = getDynamicDelay();
                    if (delay > 0 && window.quasarBotWPM < 450) {
                        if (window.quasarHumanMode) {
                            if (harf === "." || harf === "," || harf === "?" || harf === "!") {
                                await sleep(delay * 3.8);
                            } else if (typedChars % 14 === 0 && Math.random() < 0.45) {
                                await sleep(delay + randomBetween(60, 240));
                            } else {
                                await sleep(delay);
                            }
                        } else {
                        ...
                        await new Promise(window.requestAnimationFrame || (r => setTimeout(r, 0)));
                    }
                }
                ...
                if (i < kelimeler.length - 1) {
                    ...
                    const delay = getDynamicDelay();
                    if (delay > 0 && window.quasarBotWPM < 450) {
                        if (window.quasarHumanMode && Math.random() < 0.18) {
                            quasarLog(l.pause);
...
// Otomatik Başlatma Modu Butonu (⚡)
        ...
        QuasarAutomationButton.addEventListener('click', function () {
            ...
            const observer = new MutationObserver(() => {
                ...
                // 0. ANA MODELİ AÇ
                const quasarModel = document.querySelector('[data-bs-target="#odaolusturmodal"]');
                ...
                if (createBtn && !clicked.has(createBtn)) {
                    safeClick(createBtn, 100 + Math.floor(Math.random() * 100), "1. ODA OLUŞTUR");
                    return;
                }
                ...
                const offBtn = document.querySelector('.home-button[onclick="odadancik()"]');
                const finText = document.querySelector('.matchResult');
...
// --- SÜRÜKLE BIRAK SİSTEMİ ---
        ...
        QuasarPanel.addEventListener('mousedown', function(e) {
            if (e.target !== QuasarBotButton && e.target !== HizSlider && !e.target.classList.contains('quasar-utility-btn') && e.target !== LangSelect && e.target !== ControlsRow) {
                isDragging = true;
